...
public:
    ...
    static Result Parse(LiteralType, const char *s, const char *end,
                        Uint *out_bits);

private:
    static bool StringStartsWith(const char *start, const char *end,
                                 const char *prefix);
...
template <typename T>
// static
bool FloatParser<T>::StringStartsWith(const char *start, const char *end,
                                      const char *prefix) {
...
// static
template <typename T>
Result FloatParser<T>::ParseFloat(const char *s, const char *end,
                                  Uint *out_bits) {
    ...
    //
    // The WebAssembly spec also ignores underflow, so we don't need to check
    // for ERANGE at all.
    ...
    auto buffer_end = std::copy_if(s, end, buffer, [](char c) -> bool {
        return c != '_';
    });
...
// static
template <typename T>
typename FloatParser<T>::Uint FloatParser<T>::Make(bool sign, int exp,
                                                   Uint sig) {
...
// static
template <typename T>
typename FloatParser<T>::Uint
FloatParser<T>::ShiftAndRoundToNearest(Uint significand, int shift,
                                       bool seen_trailing_non_zero) {
    assert(shift > 0);
    // Round ties to even.
...
// static
template <typename T>
Result FloatParser<T>::ParseNan(const char *s, const char *end,
                                Uint *out_bits) {
    bool is_neg = false;
    if (*s == '-') {
        is_neg = true;
...
// static
template <typename T>
Result FloatParser<T>::ParseHex(const char *s, const char *end,
                                Uint *out_bits) {
    bool is_neg = false;
    if (*s == '-') {
        is_neg = true;
    ...
    if (s < end) {
        ...
        // significand_exponent_add is negated if exponent will be negative, so
        // it can be easily summed to see if the exponent is too large (see
        // below).
    ...
    if (exponent <= Traits::kMinExp) {
        ...
        if (shift <= Traits::kSigBits) {
            if (shift) {
                update_seen_trailing_non_zero(shift);
                significand = ShiftAndRoundToNearest(significand, shift,
        ...
        *out_bits = Make(is_neg, Traits::kMinExp, 0);
        ...
        if (exponent >= Traits::kMaxExp) {
            // Would be inf or -inf, but the spec doesn't allow rounding
            // hex-floats to infinity.
            return Result::Error;
        }
...
// static
template <typename T>
void FloatParser<T>::ParseInfinity(const char *s, const char *end,
                                   Uint *out_bits) {
    bool is_neg = false;
    if (*s == '-') {
        is_neg = true;
...
// static
template <typename T>
Result FloatParser<T>::Parse(LiteralType literal_type, const char *s,
                             const char *end, Uint *out_bits) {
#if COMPILER_IS_MSVC
...
void FloatWriter<T>::WriteHex(char *out, size_t size, Uint bits) {
    ...
    int exp =
        ((bits >> Traits::kSigBits) & Traits::kExpMask) - Traits::kExpBias;
        ...
        *p++ = is_zero ? '0' : '1';
        ...
        if (sig) {
            if (exp == Traits::kMinExp) {
                // Subnormal; shift the significand up, and shift out the
                // implicit 1.
...
Result ParseUint64(const char *s, const char *end, uint64_t *out) {
    ...
    if (*s == '0' && s + 1 < end && s[1] == 'x') {
        ...
        for (; s < end; ++s) {
            ...
            // Check for overflow.
            if (value > kMaxDiv16 ||
                (value == kMaxDiv16 && digit > kMaxMod16)) {
                return Result::Error;
            }
        ...
        for (; s < end; ++s) {
            ...
            // Check for overflow.
            if (value > kMaxDiv10 ||
                (value == kMaxDiv10 && digit > kMaxMod10)) {
                return Result::Error;
            }
...
Result ParseInt64(const char *s, const char *end, uint64_t *out,
...
uint32_t AddWithCarry(uint32_t x, uint32_t y, uint32_t *carry) {
    // Increments *carry if the addition overflows, otherwise leaves carry
    // alone.
    if ((0xffffffff - x) < y) {
        ++*carry;
    }
...
Result ParseUint128(const char *s, const char *end, v128 *out) {
    ...
    while (true) {
        ...
        out->set_u32(1,
                     AddWithCarry(out->u32(1), carry_into_v1, &carry_into_v2));
        out->set_u32(2,
                     AddWithCarry(out->u32(2), carry_into_v2, &carry_into_v3));
...
template <typename U>
Result ParseInt(const char *s, const char *end, U *out,
                ParseIntType parse_type) {
...
Result ParseInt8(const char *s, const char *end, uint8_t *out,
                 ParseIntType parse_type) {
    return ParseInt(s, end, out, parse_type);
}

Result ParseInt16(const char *s, const char *end, uint16_t *out,
                  ParseIntType parse_type) {
    return ParseInt(s, end, out, parse_type);
}

Result ParseInt32(const char *s, const char *end, uint32_t *out,
                  ParseIntType parse_type) {
    return ParseInt(s, end, out, parse_type);
}

Result ParseFloat(LiteralType literal_type, const char *s, const char *end,
                  uint32_t *out_bits) {
    return FloatParser<float>::Parse(literal_type, s, end, out_bits);
}

Result ParseDouble(LiteralType literal_type, const char *s, const char *end,
                   uint64_t *out_bits) {
