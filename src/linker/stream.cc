...
Result OutputBuffer::WriteToFile(StringRef filename) const {
    ...
    if (bytes < 0 || static_cast<size_t>(bytes) != data.size()) {
        ERROR0("failed to write  PRIzd  bytes to \n");
        fclose(file);
        return Result::Error;
    }
...
Result OutputBuffer::WriteToStdout() const {
    ...
    if (bytes < 0 || static_cast<size_t>(bytes) != data.size()) {
        ERROR0("failed to write  PRIzd  bytes to stdout\n");
        return Result::Error;
    }
    return Result::Ok;
}
...
FileStream::FileStream(StringRef filename, Stream *log_stream)
...
Result FileStream::WriteDataImpl(size_t at, const void *data, size_t size) {
    ...
    if (at != offset_) {
        if (fseek(file_, at, SEEK_SET) != 0) {
            ERROR0("fseek offset= PRIzd  failed, errno=\n");
            return Result::Error;
        }
        offset_ = at;
    }
    if (fwrite(data, size, 1, file_) != 1) {
        ERROR0("fwrite size= PRIzd  failed, errno=\n");
        return Result::Error;
    }
    offset_ += size;
    return Result::Ok;
