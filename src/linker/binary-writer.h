...
struct WriteBinaryOptions {
    ...
    WriteBinaryOptions(const Features &features, bool canonicalize_lebs,
                       bool relocatable, bool write_debug_names)
        : features(features), canonicalize_lebs(canonicalize_lebs),
          relocatable(relocatable), write_debug_names(write_debug_names) {
    }
...
Result WriteBinaryModule(Stream *, const WASMModule *,
