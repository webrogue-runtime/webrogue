...
public:
    ...
    inline Type() = default; // Provided so Type can be member of a union.
    inline Type(int32_t code)
        : enum_(static_cast<Enum>(code)), type_index_(kInvalidIndex) {
    }
    inline Type(Enum e) : enum_(e), type_index_(kInvalidIndex) {
    }
    inline Type(Enum e, Index type_index) : enum_(e), type_index_(type_index) {
    ...
    inline bool IsReferenceWithIndex() const {
        return enum_ == Type::Reference;
    }

    inline bool IsNullableRef() const {
    ...
    inline const char *GetRefKindName() const {
        switch (enum_) {
        case Type::FuncRef:
            return "func";
        case Type::ExternRef:
    ...
    inline bool IsIndex() const {
        return static_cast<int32_t>(enum_) >= 0;
    }
    ...
    inline Index GetReferenceIndex() const {
        assert(enum_ == Enum::Reference);
        return type_index_;
    }

    inline TypeVector GetInlineVector() const {
        assert(!IsIndex());
        switch (enum_) {
        case Type::Void:
            return TypeVector();
