...
void WriteStr(Stream *stream, StringRef s, const char *desc,
              PrintChars print_chars) {
    WriteU32Leb128(stream, s.size(), "string length");
    stream->WriteData(s.data(), s.size(), desc, print_chars);
}
...
void WriteDebugName(Stream *stream, StringRef name, const char *desc) {
    StringRef stripped_name = name;
    if (!stripped_name.empty()) {
        // Strip leading $ from name
        assert(stripped_name.front() == '$');
        stripped_name = stripped_name.drop_front(1);
    }
...
public:
    Symbol(const StringRef &name, uint8_t flags, const Function &f)
        : type_(Function::type), name_(name), flags_(flags), function_(f) {
    }
    Symbol(const StringRef &name, uint8_t flags, const Data &d)
        : type_(Data::type), name_(name), flags_(flags), data_(d) {
    }
    Symbol(const StringRef &name, uint8_t flags, const Global &g)
        : type_(Global::type), name_(name), flags_(flags), global_(g) {
    }
    Symbol(const StringRef &name, uint8_t flags, const Section &s)
        : type_(Section::type), name_(name), flags_(flags), section_(s) {
    }
    Symbol(const StringRef &name, uint8_t flags, const Tag &e)
        : type_(Tag::type), name_(name), flags_(flags), tag_(e) {
    }
    Symbol(const StringRef &name, uint8_t flags, const Table &t)
    ...
    SymbolVisibility visibility() const {
        return static_cast<SymbolVisibility>(flags() &
    ...
    bool undefined() const {
        return flags() & WABT_SYMBOL_FLAG_UNDEFINED;
    }
    bool defined() const {
        return !undefined();
    }
    bool exported() const {
        return flags() & WABT_SYMBOL_FLAG_EXPORTED;
    }
    bool explicit_name() const {
        return flags() & WABT_SYMBOL_FLAG_EXPLICIT_NAME;
    }
    bool no_strip() const {
        return flags() & WABT_SYMBOL_FLAG_NO_STRIP;
    }
...
class SymbolTable {
    ...
    std::set<StringRef> seen_names_;

    Result EnsureUnique(const StringRef &name) {
    ...
    template <typename T>
    Result AddSymbol(std::vector<Index> *map, StringRef name, bool imported,
                     bool exported, T &&sym) {
        uint8_t flags = 0;
        if (imported) {
            flags |= WABT_SYMBOL_FLAG_UNDEFINED;
            // Wabt currently has no way for a user to explicitly specify the
            // name of an import, so never set the EXPLICIT_NAME flag, and
            // ignore any display name fabricated by wabt.
            name = StringRef();
        } else {
            if (name.empty()) {
                ...
                // Otherwise, strip the dollar off the name; a definition $foo
                // is available for linking as "foo".
                assert(name[0] == '$');
    ...
    Index SymbolIndex(const std::vector<Index> &table, Index index) const {
        // For well-formed modules, an index into (e.g.) functions_ will always
        // be within bounds; the out-of-bounds case here is just to allow
        // --relocatable to write known-invalid modules.
...
public:
    ...
    Result Populate(const WASMModule *module) {
        ...
        // We currently only create symbol table entries for function, table,
        // and global symbols.
...
using CodeMetadataSections = std::map<StringRef, CodeMetadataSection>;
...
private:
    ...
    Offset WriteFixupU32Leb128Size(Offset offset, Offset leb_size_guess,
                                   const char *desc);
    void BeginKnownSection(BinarySection section_code);
    void BeginCustomSection(const char *name);
    void WriteSectionHeader(BinarySection section_code);
    void EndSection();
    ...
    void WriteU32Leb128WithReloc(Index index, const char *desc,
                                 RelocType reloc_type);
    void WriteS32Leb128WithReloc(int32_t value, const char *desc,
                                 RelocType reloc_type);
    void WriteTableNumberWithReloc(Index table_number, const char *desc);
    template <typename T>
    void WriteLoadStoreExpr(const Func *func, const Expr *expr,
                            const char *desc);
    template <typename T>
    void WriteSimdLoadStoreLaneExpr(const Func *func, const Expr *expr,
                                    const char *desc);
...
void BinaryWriter::WriteSectionHeader(BinarySection section_code) {
...
void BinaryWriter::BeginKnownSection(BinarySection section_code) {
    WriteSectionHeader(section_code);
}

void BinaryWriter::BeginCustomSection(const char *name) {
    WriteSectionHeader(BinarySection::Custom);
...
void BinaryWriter::AddReloc(RelocType reloc_type, Index index) {
    ...
    if (symbol_index == kInvalidIndex) {
        // The file is invalid, for example a reference to function 42 where
        // only 10 functions are defined.  The user must have already passed
...
void BinaryWriter::WriteU32Leb128WithReloc(Index index, const char *desc,
                                           RelocType reloc_type) {
...
void BinaryWriter::WriteS32Leb128WithReloc(int32_t value, const char *desc,
                                           RelocType reloc_type) {
    if (options_.relocatable) {
...
void BinaryWriter::WriteSimdLoadStoreLaneExpr(const Func *func,
    ...
    auto *typed_expr = cast<T>(expr);
    stream_->WriteU8(static_cast<uint8_t>(typed_expr->val),
                     "Simd Lane literal");
}
...
void BinaryWriter::WriteExpr(const Func *func, const Expr *expr) {
    ...
    case ExprType::GlobalGet: {
        ...
        WriteU32Leb128WithReloc(index, "global index",
        ...
        case TryKind::Catch:
            for (const Catch &catch_ : try_expr->catches) {
                if (catch_.IsCatchAll()) {
                    ...
                    WriteU32Leb128(stream_, GetTagVarDepth(&catch_.var),
                                   "catch tag");
                }
    ...
    case ExprType::SimdStoreLane: {
        WriteSimdLoadStoreLaneExpr<SimdStoreLaneExpr>(func, expr,
                                                      "store offset");
        break;
    }
...
void BinaryWriter::WriteRelocSection(const RelocSection *reloc_section) {
    std::string section_name =
        WABT_BINARY_SECTION_RELOC + std::string(".") + reloc_section->name;
    BeginCustomSection(section_name.c_str());
    WriteU32Leb128(stream_, reloc_section->section_index,
                   "reloc section index");
    ...
    if (symbols.size()) {
        ...
        for (const Symbol &sym : symbols) {
            ...
            case SymbolType::Function:
                WriteU32Leb128(stream_, sym.AsFunction().index,
                               "function index");
                if (sym.defined() || sym.explicit_name()) {
                    WriteStr(stream_, sym.name(), "function name",
                             PrintChars::Yes);
                }
                break;
            ...
            case SymbolType::Global:
                ...
                if (sym.defined() || sym.explicit_name()) {
                    WriteStr(stream_, sym.name(), "global name",
                             PrintChars::Yes);
                }
                break;
            case SymbolType::Section:
                WriteU32Leb128(stream_, sym.AsSection().section,
                               "section index");
                break;
            case SymbolType::Tag:
            ...
            case SymbolType::Table:
                ...
                if (sym.defined() || sym.explicit_name()) {
                    WriteStr(stream_, sym.name(), "table name",
                             PrintChars::Yes);
                }
                break;
            }
...
void BinaryWriter::WriteNames(const std::vector<T *> &elems,
    ...
    for (size_t i = 0; i < elems.size(); ++i) {
        ...
        WriteDebugName(stream_, elem->name, "elem name %");
    }
    EndSubsection();
}
...
Result BinaryWriter::WriteModule() {
    ...
    if (module_->types.size()) {
        ...
        for (size_t i = 0; i < module_->types.size(); ++i) {
            ...
            case TypeEntryKind::Array: {
                ...
                stream_->WriteU8(array_type->field.mutable_,
                                 "field mutability");
                break;
            }
            }
    ...
    if (num_funcs) {
        ...
        for (size_t i = 0; i < num_funcs; ++i) {
            ...
            WriteU32Leb128(stream_, module_->GetFuncTypeIndex(func->decl),
                           "function % PRIzd  signature index");
        }
        EndSection();
    }
    ...
    if (num_tables) {
        ...
        for (size_t i = 0; i < num_tables; ++i) {
            const Table *table =
                module_->tables[i + module_->num_table_imports];
            WriteHeader("table", i);
            WriteTable(table);
        }
        EndSection();
    ...
    if (num_memories) {
        ...
        for (size_t i = 0; i < num_memories; ++i) {
            const Memory *memory =
                module_->memories[i + module_->num_memory_imports];
    ...
    if (num_globals) {
        ...
        for (size_t i = 0; i < num_globals; ++i) {
            const Global *global =
                module_->globals[i + module_->num_global_imports];
    ...
    if (module_->elem_segments.size()) {
        ...
        WriteU32Leb128(stream_, module_->elem_segments.size(),
                       "num elem segments");
        for (size_t i = 0; i < module_->elem_segments.size(); ++i) {
            ...
            if (flags & SegExplicitIndex &&
                segment->kind != SegmentKind::Declared) {
                WriteU32Leb128(stream_,
                               module_->GetTableIndex(segment->table_var),
                               "table index");
            }
            // 3. optional target location within the table (active segments
            // only)
            if (!(flags & SegPassive)) {
                WriteInitExpr(segment->offset);
            }
            // 4. type of item in the following list (omitted for "legacy"
            // segments)
    ...
    if (num_funcs) {
        ...
        if (module_->code.empty() && false) {
            for (size_t i = 0; i < num_funcs; ++i) {
                cur_func_index_ = i + module_->num_func_imports;
                WriteHeader("function body", i);
                const Func *func = module_->funcs[cur_func_index_];

                /* TODO(binji): better guess of the size of the function body
                 * section */
                const Offset leb_size_guess = 1;
                Offset body_size_offset = WriteU32Leb128Space(
                    leb_size_guess, "func body size (guess)");
                cur_func_start_offset_ = stream_->offset();
                WriteFunc(func);
                auto func_start_offset =
                    body_size_offset - last_section_payload_offset_;
                auto func_end_offset =
                    stream_->offset() - last_section_payload_offset_;
                auto delta = WriteFixupU32Leb128Size(
                    body_size_offset, leb_size_guess, "FIXUP func body size");
            ...
            stream_->WriteData(module_->code.data(), module_->code.size(),
                               "Code section");
        }
        EndSection();
    }
    ...
    if (module_->data_segments.size()) {
        ...
        WriteU32Leb128(stream_, module_->data_segments.size(),
                       "num data segments");
        for (size_t i = 0; i < module_->data_segments.size(); ++i) {
            ...
            if (!(flags & SegPassive)) {
                if (options_.features.multi_memory_enabled() &&
                    ...
                    WriteU32Leb128(stream_,
                                   module_->GetMemoryIndex(segment->memory_var),
                                   "memidx");
                } else {
    ...
    for (const Custom &custom : module_->customs) {
        // These custom sections are already specially handled by BinaryWriter,
        // so we don't want to double-write.
        if ((custom.name == WABT_BINARY_SECTION_NAME &&
            ...
            (custom.name == WABT_BINARY_SECTION_LINKING &&
             options_.relocatable) ||
...
Result WriteBinaryModule(Stream *stream, const WASMModule *module,
