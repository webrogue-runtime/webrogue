...
#undef WABT_OPCODE
    ...
    Type GetResultType() const {
        return GetInfo().result_type;
    }
    Type GetParamType1() const {
        return GetInfo().param_types[0];
    }
    Type GetParamType2() const {
        return GetInfo().param_types[1];
    }
    Type GetParamType3() const {
        return GetInfo().param_types[2];
    }
    Type GetParamType(int n) const {
        return GetInfo().param_types[n - 1];
    }
    Address GetMemorySize() const {
        return GetInfo().memory_size;
    }
...
private:
    ...
    static void DecodeInvalidOpcode(Enum e, uint8_t *out_prefix,
                                    uint32_t *out_code) {
...
inline Opcode Opcode::FromCode(uint8_t prefix, uint32_t code) {
    ...
    if (WABT_LIKELY(prefix_code < WABT_ARRAY_SIZE(WabtOpcodeCodeTable))) {
        ...
        // The default value in the table is 0. That's a valid value, but only
        // if the code is 0 (for nop).
