...
namespace wabt {
inline void MemcpyEndianAware(void *dst, const void *src, size_t dsize,
                              size_t ssize, size_t doff, size_t soff,
                              size_t len) {
#if WABT_BIG_ENDIAN
...
struct v128 {
    ...
    uint8_t u8(int lane) const {
        return To<uint8_t>(lane);
    }
    uint16_t u16(int lane) const {
        return To<uint16_t>(lane);
    }
    uint32_t u32(int lane) const {
        return To<uint32_t>(lane);
    }
    uint64_t u64(int lane) const {
        return To<uint64_t>(lane);
    }
    uint32_t f32_bits(int lane) const {
        return To<uint32_t>(lane);
    }
    uint64_t f64_bits(int lane) const {
        return To<uint64_t>(lane);
    }

    void set_u8(int lane, uint8_t x) {
        return From<uint8_t>(lane, x);
    }
    void set_u16(int lane, uint16_t x) {
        return From<uint16_t>(lane, x);
    }
    void set_u32(int lane, uint32_t x) {
        return From<uint32_t>(lane, x);
    }
    void set_u64(int lane, uint64_t x) {
        return From<uint64_t>(lane, x);
    }
    void set_f32_bits(int lane, uint32_t x) {
        return From<uint32_t>(lane, x);
    }
    void set_f64_bits(int lane, uint64_t x) {
        return From<uint64_t>(lane, x);
    }

    bool is_zero() const {
        return std::all_of(std::begin(v), std::end(v), [](uint8_t x) {
            return x == 0;
        });
    }
    void set_zero() {
    ...
    template <typename T> void From(int lane, T data) {
        static_assert(sizeof(T) <= sizeof(v), "Invalid cast!");
        assert((lane + 1) * sizeof(T) <= sizeof(v));
        wabt::MemcpyEndianAware(v, &data, sizeof(v), sizeof(data),
                                lane * sizeof(T), 0, sizeof(data));
    }
...
// Placement destruct
template <typename T> void Destruct(T &placement) {
    placement.~T();
}
...
enum SegmentFlags : uint8_t {
    ...
    SegExplicitIndex =
        2,               // bit 1: Has explict index (Implies table 0 if absent)
...
struct Limits {
    ...
    Type IndexType() const {
        return is_64 ? Type::I64 : Type::I32;
    }
...
Result ReadFile(StringRef filename, std::vector<uint8_t> *out_data);
