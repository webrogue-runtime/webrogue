...
const Export *WASMModule::GetExport(StringRef name) const {
...
Index WASMModule::GetFuncIndex(const Var &var) const {
    return func_bindings.FindIndex(var);
}

Index WASMModule::GetGlobalIndex(const Var &var) const {
    return global_bindings.FindIndex(var);
}

Index WASMModule::GetTableIndex(const Var &var) const {
    return table_bindings.FindIndex(var);
}

Index WASMModule::GetMemoryIndex(const Var &var) const {
    return memory_bindings.FindIndex(var);
}

Index WASMModule::GetFuncTypeIndex(const Var &var) const {
    return type_bindings.FindIndex(var);
}

Index WASMModule::GetTagIndex(const Var &var) const {
    return tag_bindings.FindIndex(var);
}

Index WASMModule::GetDataSegmentIndex(const Var &var) const {
    return data_segment_bindings.FindIndex(var);
}

Index WASMModule::GetElemSegmentIndex(const Var &var) const {
    return elem_segment_bindings.FindIndex(var);
}

bool WASMModule::IsImport(ExternalKind kind, const Var &var) const {
    switch (kind) {
...
Index LocalTypes::size() const {
    return std::accumulate(decls_.begin(), decls_.end(), 0,
                           [](Index sum, const Decl &decl) {
...
const Func *WASMModule::GetFunc(const Var &var) const {
    return const_cast<WASMModule *>(this)->GetFunc(var);
}

Func *WASMModule::GetFunc(const Var &var) {
...
const Global *WASMModule::GetGlobal(const Var &var) const {
    return const_cast<WASMModule *>(this)->GetGlobal(var);
}

Global *WASMModule::GetGlobal(const Var &var) {
...
const Table *WASMModule::GetTable(const Var &var) const {
    return const_cast<WASMModule *>(this)->GetTable(var);
}

Table *WASMModule::GetTable(const Var &var) {
...
const Memory *WASMModule::GetMemory(const Var &var) const {
    return const_cast<WASMModule *>(this)->GetMemory(var);
}

Memory *WASMModule::GetMemory(const Var &var) {
...
Tag *WASMModule::GetTag(const Var &var) const {
...
const DataSegment *WASMModule::GetDataSegment(const Var &var) const {
    return const_cast<WASMModule *>(this)->GetDataSegment(var);
}

DataSegment *WASMModule::GetDataSegment(const Var &var) {
...
const ElemSegment *WASMModule::GetElemSegment(const Var &var) const {
    return const_cast<WASMModule *>(this)->GetElemSegment(var);
}

ElemSegment *WASMModule::GetElemSegment(const Var &var) {
...
const FuncType *WASMModule::GetFuncType(const Var &var) const {
    return const_cast<WASMModule *>(this)->GetFuncType(var);
}

FuncType *WASMModule::GetFuncType(const Var &var) {
...
Index WASMModule::GetFuncTypeIndex(const FuncSignature &sig) const {
...
Index WASMModule::GetFuncTypeIndex(const FuncDeclaration &decl) const {
...
void WASMModule::AppendField(std::unique_ptr<DataSegmentModuleField> field) {
    DataSegment &data_segment = field->data_segment;
    if (!data_segment.name.empty()) {
        data_segment_bindings.emplace(data_segment.name,
                                      Binding(data_segments.size()));
    }
    data_segments.push_back(&data_segment);
    fields.push_back(std::move(field));
}

void WASMModule::AppendField(std::unique_ptr<ElemSegmentModuleField> field) {
    ElemSegment &elem_segment = field->elem_segment;
    if (!elem_segment.name.empty()) {
        elem_segment_bindings.emplace(elem_segment.name,
                                      Binding(elem_segments.size()));
    }
    elem_segments.push_back(&elem_segment);
    fields.push_back(std::move(field));
}

void WASMModule::AppendField(std::unique_ptr<TagModuleField> field) {
    Tag &tag = field->tag;
    if (!tag.name.empty()) {
        tag_bindings.emplace(tag.name, Binding(tags.size()));
    }
    tags.push_back(&tag);
    fields.push_back(std::move(field));
}

void WASMModule::AppendField(std::unique_ptr<ExportModuleField> field) {
    // Exported names are allowed to be empty.
    Export &export_ = field->export_;
    export_bindings.emplace(export_.name, Binding(exports.size()));
    exports.push_back(&export_);
    fields.push_back(std::move(field));
}

void WASMModule::AppendField(std::unique_ptr<FuncModuleField> field) {
    Func &func = field->func;
    if (!func.name.empty()) {
        func_bindings.emplace(func.name, Binding(funcs.size()));
    }
    funcs.push_back(&func);
    fields.push_back(std::move(field));
}

void WASMModule::AppendField(std::unique_ptr<TypeModuleField> field) {
    TypeEntry &type = *field->type;
    if (!type.name.empty()) {
        type_bindings.emplace(type.name, Binding(types.size()));
    }
    types.push_back(&type);
    fields.push_back(std::move(field));
}

void WASMModule::AppendField(std::unique_ptr<GlobalModuleField> field) {
    Global &global = field->global;
    if (!global.name.empty()) {
        global_bindings.emplace(global.name, Binding(globals.size()));
    }
    globals.push_back(&global);
    fields.push_back(std::move(field));
}

void WASMModule::AppendField(std::unique_ptr<ImportModuleField> field) {
    ...
    if (!name->empty()) {
        bindings->emplace(*name, Binding(index));
    }
    imports.push_back(import);
    fields.push_back(std::move(field));
}

void WASMModule::AppendField(std::unique_ptr<MemoryModuleField> field) {
    Memory &memory = field->memory;
    if (!memory.name.empty()) {
        memory_bindings.emplace(memory.name, Binding(memories.size()));
    }
    memories.push_back(&memory);
    fields.push_back(std::move(field));
}

void WASMModule::AppendField(std::unique_ptr<StartModuleField> field) {
    starts.push_back(&field->start);
    fields.push_back(std::move(field));
}

void WASMModule::AppendField(std::unique_ptr<TableModuleField> field) {
    Table &table = field->table;
    if (!table.name.empty()) {
        table_bindings.emplace(table.name, Binding(tables.size()));
    }
    tables.push_back(&table);
    fields.push_back(std::move(field));
}

void WASMModule::AppendField(std::unique_ptr<ModuleField> field) {
    switch (field->type()) {
...
void WASMModule::AppendFields(ModuleFieldList *fields) {
    while (!fields->empty())
        AppendField(std::unique_ptr<ModuleField>(fields->extract_front()));
}

const WASMModule *Script::GetFirstModule() const {
    return const_cast<Script *>(this)->GetFirstModule();
}

WASMModule *Script::GetFirstModule() {
...
const WASMModule *Script::GetModule(const Var &var) const {
...
void MakeTypeBindingReverseMapping(
    ...
    for (const auto &value : bindings) {
        assert(static_cast<size_t>(value.second.index) <
               out_reverse_mapping->size());
        (*out_reverse_mapping)[value.second.index] = value.first;
    }
}

Var::Var() : Var(kInvalidIndex) {
}

Var::Var(Index index) : type_(VarType::Index), index_(index) {
}

Var::Var(StringRef name) : type_(VarType::Name), name_(name.str()) {
}
...
void Var::set_name(StringRef name) {
    set_name(name.str());
}
...
WASMModule::Symbol::~Symbol() {
}

WASMModule::FuncSymbol::~FuncSymbol() {
}
WASMModule::DataSymbol::~DataSymbol() {
}
WASMModule::GlobalSymbol::~GlobalSymbol() {
}
WASMModule::SectionSymbol::~SectionSymbol() {
}
WASMModule::TagSymbol::~TagSymbol() {
}
WASMModule::TableSymbol::~TableSymbol() {
}

uint8_t ElemSegment::GetFlags(const WASMModule *module) const {
    uint8_t flags = 0;
...
uint8_t DataSegment::GetFlags(const WASMModule *module) const {
    uint8_t flags = 0;
