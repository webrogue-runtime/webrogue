...
struct Label {
    Label(LabelType label_type, const std::string &name,
          const TypeVector &param_types, const TypeVector &result_types)
        : name(name), label_type(label_type), param_types(param_types),
...
struct ModuleContext {
    ModuleContext(const WASMModule &module) : module(module) {
    }

    Index GetLabelStackSize() const {
        return label_stack_.size();
    }
