...
public:
    ...
    [[nodiscard]] int compare(StringRef RHS) const {
        ...
        if (int Res =
                compareMemory(Data, RHS.Data, std::min(Length, RHS.Length)))
            return Res < 0 ? -1 : 1;
    ...
    /// found.
    [[nodiscard]] inline size_t find(char C, size_t From = 0) const {
    ...
    /// found.
    [[nodiscard]] inline size_t find(StringRef Str, size_t From = 0) const {
        if (From > Length)
            return npos;
        ...
        if (N == 2) {
            // Provide a fast path for newline finding (CRLF case) in
            // InclusionRewriter. Not the most optimized strategy, but getting
            // memcmp inlined should be good enough.
            do {
        ...
        do {
            ...
            if (WABT_UNLIKELY(Last == (uint8_t)Needle[N - 1]))
    ...
    template <typename T> bool getAsInteger(unsigned Radix, T &Result) const {
        if (std::numeric_limits<T>::is_signed) {
            ...
            // The additional cast to unsigned long long is required to avoid
            // the Visual C++ warning C4805: '!=' : unsafe mix of type 'bool'
            // and type 'unsigned __int64' when instantiating getAsInteger with
            // T = bool.
            if (getAsUnsignedInteger(*this, Radix, ULLVal) ||
                static_cast<unsigned long long>(static_cast<T>(ULLVal)) !=
                    ULLVal)
                return true;
            Result = ULLVal;
        }
