...
struct ReadBinaryOptions {
    ...
    ReadBinaryOptions(const Features &features, Stream *logStream,
                      bool readDebugNames, bool stopOnFirstError,
                      bool failOnCustomSectionError)
        : features(features), logStream(logStream),
          readDebugNames(readDebugNames), stopOnFirstError(stopOnFirstError),
          failOnCustomSectionError(failOnCustomSectionError) {
    }

    Features features;
    Stream *logStream = nullptr;
    bool readDebugNames = false;
    bool stopOnFirstError = true;
    bool failOnCustomSectionError = true;
};
...
public:
    ...
    virtual Result BeginSection(Index section_index, BinarySection section_type,
                                Offset size) = 0;

    /* Custom section */
    virtual Result BeginCustomSection(Index section_index, Offset size,
                                      StringRef section_name) = 0;
    virtual Result EndCustomSection() = 0;

    /* Type section */
    virtual Result BeginTypeSection(Offset size) = 0;
    virtual Result OnTypeCount(Index count) = 0;
    virtual Result OnFuncType(Index index, Index param_count, Type *param_types,
                              Index result_count, Type *result_types) = 0;
    virtual Result OnStructType(Index index, Index field_count,
                                TypeMut *fields) = 0;
    ...
    virtual Result OnImport(Index index, ExternalKind kind,
                            StringRef module_name, StringRef field_name) = 0;
    virtual Result OnImportFunc(Index import_index, StringRef module_name,
                                StringRef field_name, Index func_index,
                                Index sig_index) = 0;
    virtual Result OnImportTable(Index import_index, StringRef module_name,
                                 StringRef field_name, Index table_index,
                                 Type elem_type, const Limits *elem_limits) = 0;
    virtual Result OnImportMemory(Index import_index, StringRef module_name,
                                  StringRef field_name, Index memory_index,
                                  const Limits *page_limits) = 0;
    virtual Result OnImportGlobal(Index import_index, StringRef module_name,
                                  StringRef field_name, Index global_index,
                                  Type type, bool mutable_) = 0;
    virtual Result OnImportTag(Index import_index, StringRef module_name,
                               StringRef field_name, Index tag_index,
                               Index sig_index) = 0;
    ...
    virtual Result OnTable(Index index, Type elem_type,
    ...
    virtual Result OnExport(Index index, ExternalKind kind, Index item_index,
                            StringRef name) = 0;
    ...
    virtual Result OnOpcodeUint32Uint32Uint32(uint32_t value, uint32_t value2,
                                              uint32_t value3) = 0;
    ...
    virtual Result OnAtomicLoadExpr(Opcode opcode, Index memidx,
                                    Address alignment_log2, Address offset) = 0;
    virtual Result OnAtomicStoreExpr(Opcode opcode, Index memidx,
                                     Address alignment_log2,
                                     Address offset) = 0;
    virtual Result OnAtomicRmwExpr(Opcode opcode, Index memidx,
                                   Address alignment_log2, Address offset) = 0;
    virtual Result OnAtomicRmwCmpxchgExpr(Opcode opcode, Index memidx,
                                          Address alignment_log2,
                                          Address offset) = 0;
    virtual Result OnAtomicWaitExpr(Opcode opcode, Index memidx,
                                    Address alignment_log2, Address offset) = 0;
    virtual Result OnAtomicFenceExpr(uint32_t consistency_model) = 0;
    virtual Result OnAtomicNotifyExpr(Opcode opcode, Index memidx,
                                      Address alignment_log2,
                                      Address offset) = 0;
    ...
    virtual Result OnBrTableExpr(Index num_targets, Index *target_depths,
    ...
    virtual Result OnLoadExpr(Opcode opcode, Index memidx,
                              Address alignment_log2, Address offset) = 0;
    ...
    virtual Result OnStoreExpr(Opcode opcode, Index memidx,
                               Address alignment_log2, Address offset) = 0;
    ...
    virtual Result OnSimdLoadLaneExpr(Opcode opcode, Index memidx,
                                      Address alignment_log2, Address offset,
                                      uint64_t value) = 0;
    virtual Result OnSimdStoreLaneExpr(Opcode opcode, Index memidx,
                                       Address alignment_log2, Address offset,
                                       uint64_t value) = 0;

    virtual Result OnLoadSplatExpr(Opcode opcode, Index memidx,
                                   Address alignment_log2, Address offset) = 0;
    virtual Result OnLoadZeroExpr(Opcode opcode, Index memidx,
                                  Address alignment_log2, Address offset) = 0;
    ...
    virtual Result OnDataSegmentData(Index index, const void *data,
                                     Address size) = 0;
    ...
    virtual Result OnModuleNameSubsection(Index index, uint32_t name_type,
                                          Offset subsection_size) = 0;
    virtual Result OnModuleName(StringRef name) = 0;
    virtual Result OnFunctionNameSubsection(Index index, uint32_t name_type,
                                            Offset subsection_size) = 0;
    ...
    virtual Result OnLocalNameSubsection(Index index, uint32_t name_type,
                                         Offset subsection_size) = 0;
    virtual Result OnLocalNameFunctionCount(Index num_functions) = 0;
    virtual Result OnLocalNameLocalCount(Index function_index,
                                         Index num_locals) = 0;
    virtual Result OnLocalName(Index function_index, Index local_index,
                               StringRef local_name) = 0;
    ...
    virtual Result OnNameEntry(NameSectionSubsection type, Index index,
                               StringRef name) = 0;
    ...
    virtual Result OnReloc(RelocType type, Offset offset, Index index,
                           uint32_t addend) = 0;
    virtual Result EndRelocSection() = 0;

    /* Dylink section */
    virtual Result BeginDylinkSection(Offset size) = 0;
    virtual Result OnDylinkInfo(uint32_t mem_size, uint32_t mem_align_log2,
                                uint32_t table_size,
    ...
    virtual Result OnDylinkImport(StringRef module, StringRef name,
                                  uint32_t flags) = 0;
    virtual Result OnDylinkExport(StringRef name, uint32_t flags) = 0;
    virtual Result OnDylinkNeededCount(Index count) = 0;
    virtual Result OnDylinkNeeded(StringRef so_name) = 0;
    ...
    virtual Result OnFeature(uint8_t prefix, StringRef name) = 0;
    virtual Result EndTargetFeaturesSection() = 0;

    /* Generic custom section */
    virtual Result BeginGenericCustomSection(Offset size) = 0;
    virtual Result OnGenericCustomSection(StringRef name, const void *data,
                                          Offset size) = 0;
    ...
    virtual Result OnDataSymbol(Index index, uint32_t flags, StringRef name,
                                Index segment, uint32_t offset,
                                uint32_t size) = 0;
    virtual Result OnFunctionSymbol(Index index, uint32_t flags, StringRef name,
                                    Index function_index) = 0;
    virtual Result OnGlobalSymbol(Index index, uint32_t flags, StringRef name,
                                  Index global_index) = 0;
    virtual Result OnSectionSymbol(Index index, uint32_t flags,
                                   Index section_index) = 0;
    virtual Result OnTagSymbol(Index index, uint32_t flags, StringRef name,
                               Index tag_index) = 0;
    virtual Result OnTableSymbol(Index index, uint32_t flags, StringRef name,
                                 Index table_index) = 0;
    virtual Result OnSegmentInfoCount(Index count) = 0;
    virtual Result OnSegmentInfo(Index index, StringRef name,
                                 Address alignment_log2, uint32_t flags) = 0;
    ...
    virtual Result OnComdatBegin(StringRef name, uint32_t flags,
                                 Index count) = 0;
    ...
    virtual Result BeginCodeMetadataSection(StringRef name, Offset size) = 0;
    ...
    virtual Result OnCodeMetadata(Offset offset, const void *data,
                                  Address size) = 0;
...
Result ReadBinary(const void *data, size_t size, BinaryReaderDelegate *reader,
                  const ReadBinaryOptions &options);

size_t ReadU32Leb128(const uint8_t *ptr, const uint8_t *end,
                     uint32_t *out_value);

size_t ReadI32Leb128(const uint8_t *ptr, const uint8_t *end,
                     uint32_t *out_value);
