...
// Same as above but taking a StringRef
inline Result ParseInt8(StringRef v, uint8_t *out, ParseIntType parse_type) {
    return ParseInt8(v.data(), v.data() + v.size(), out, parse_type);
}

inline Result ParseInt16(StringRef v, uint16_t *out, ParseIntType parse_type) {
    return ParseInt16(v.data(), v.data() + v.size(), out, parse_type);
}

inline Result ParseInt32(StringRef v, uint32_t *out, ParseIntType parse_type) {
    return ParseInt32(v.data(), v.data() + v.size(), out, parse_type);
}

inline Result ParseInt64(StringRef v, uint64_t *out, ParseIntType parse_type) {
    return ParseInt64(v.data(), v.data() + v.size(), out, parse_type);
}

inline Result ParseUint64(StringRef v, uint64_t *out) {
    return ParseUint64(v.data(), v.data() + v.size(), out);
}

inline Result ParseUint128(StringRef v, v128 *out) {
    return ParseUint128(v.data(), v.data() + v.size(), out);
}

inline Result ParseFloat(LiteralType literal_type, StringRef v,
                         uint32_t *out_bits) {
    return ParseFloat(literal_type, v.data(), v.data() + v.size(), out_bits);
}

inline Result ParseDouble(LiteralType literal_type, StringRef v,
                          uint64_t *out_bits) {
