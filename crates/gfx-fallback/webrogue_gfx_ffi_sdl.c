...
void *webrogue_gfx_ffi_create_window(void *raw_system_ptr) {
...
#endif
    window_ptr->sdl_window = SDL_CreateWindow("webrogue", 800, 450,
    ...
    window_ptr->display = eglGetPlatformDisplay(EGL_PLATFORM_ANGLE_ANGLE, NULL, NULL);
    ...
    EGLConfig config;
    EGLint numConfigs;
    if(!eglChooseConfig(window_ptr->display, &configAttribs, &config, 1, &numConfigs)) {
    ...
    window_ptr->context = eglCreateContext(window_ptr->display, config, NULL, &contextAttribs);
    if(!window_ptr->context) {
        printf("eglCreateContext() returned error %d\n", eglGetError());
        abort();
    }
    
    window_ptr->surface = eglCreateWindowSurface(window_ptr->display, config, (EGLNativeWindowType) window_ptr->metal_layer, NULL);
...
#ifdef WEBROGUE_GFX_IOS
    eglMakeCurrent(window_ptr->display, window_ptr->surface, window_ptr->surface, window_ptr->context);
#endif
