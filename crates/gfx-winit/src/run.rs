...
impl Mailbox {
    pub fn execute<Output, F: FnOnce(&dyn ActiveEventLoop) -> Output + Send>(&self, func: F) -> Output
    where
        Output: Send + 'static,
    {
...
pub fn run<Output>(body_fn: impl FnOnce(WinitSystem) -> Output + Send + 'static) -> Output
where
    Output: Send + 'static,
{
