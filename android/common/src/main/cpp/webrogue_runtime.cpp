...
extern "C" void webrogue_android_print(char *str, size_t len) {
  ...
  jmethodID print_bytes_method_id =
      jniEnv->GetMethodID(webrogue_activity_class, "printBytes", "([B)V");
  ...
  jniEnv->CallVoidMethodA(webrogue_activity, print_bytes_method_id,
                                (const jvalue *)&arg);
}
...
extern "C" const char *webrogue_android_path() {
  ...
  auto returned_string = static_cast<jstring>(jniEnv->CallObjectMethod(
          webrogue_activity, get_wrapp_path_method_id));
  // leaks!
  jboolean isCopy = 1;
